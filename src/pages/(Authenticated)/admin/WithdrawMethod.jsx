...
const WithdrawMethod = () => {
  ...
  return (
    <AdminPage
      ...
      subtitle="Control all withdrawal gateways, fee settings, and method availability."
      actions={
    ...
    >
      ...
      {isLoading ? (
        <AdminCard>
          <LoadingState text="Loading withdraw methods..." />
        </AdminCard>
      ) : null}
      {isError ? (
        <AdminCard>
          <ErrorState text="Error loading withdraw methods." />
        </AdminCard>
      ) : null}
      ...
      {!isLoading && !isError && apiData?.length === 0 ? (
        <AdminCard>
          <EmptyState text="No withdraw methods configured yet." />
        </AdminCard>
      ) : null}

      {isDialogOpen && (
        <AdminModal title={editingMethod ? "Update Withdraw Method" : "Add Withdraw Method"} onClose={() => {
          setIsDialogOpen(false);
          setEditingMethod(null);
        }}>
            <form onSubmit={formik.handleSubmit} className="grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-3">
              {[
