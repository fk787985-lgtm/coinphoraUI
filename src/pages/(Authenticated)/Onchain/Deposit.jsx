...
const Deposit = () => {
  ...
  const handleCardClick = (card) => {
    navigate(`/deposit/checkout/${card._id}`, {
      state: {
        name: `${card.paymentAddressType.toUpperCase()}`,
        label: `Deposit with  ${card.coinName}`,
        address: card.paymentAddress,
        minDeposit: card.minAmount,
        maxDeposit: card.maxAmount,
  ...
  if (isError) return <div className="text-red-500 p-4">Failed to load deposit methods.</div>;
          ...
          >
            ...
            <div className="flex flex-col ml-3">
              ...
              <span className="text-xs text-gray-400">
                Deposit with currency: {" "} {card.paymentAddressType.toUpperCase()}
              </span>
            </div>
